...
Run command `build` and then `deploy`. You will see lambda function and DynamoDB table resources are being created automatically. After the deployment is successfully completed you can hit the `todo` endpoint using postman to create a new Todo. If you get a successful response, then you can verify the same details in the DyanmoDB table.