...
# Fetch todos
## Get all
To get all todos we have created, we can use the `scan` method of the DyanmoDB client. Let's add a function `getAllTodos` to get all todos inside an array.
```ts showLineNumbers
...
To get a single todo we are using `get` method of `docClient` object. Parameters for the `get` method are `TableName` and an object assigned to `Key` which has todo's **"id"**. Now add the API gateway handler for `getTodoById` function.
```ts showLineNumbers
